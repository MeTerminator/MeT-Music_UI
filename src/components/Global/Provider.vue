...
const { themeType, themeAuto, themeTypeName, themeTypeData, themeAutoCover, siteFont, lyricFont } =
...
const fontConfigs = {
  ...
  harmony_bold: { family: '"HarmonyOS_Bold", "HarmonyOS_Regular", system-ui', weight: 'bold' },
  pingfang_reg: { family: '"PingFangSC_Regular", "SF_Pro_Regular", system-ui', weight: 'normal' },
  pingfang_semi: { family: '"PingFangSC_Semibold", "SF_Pro_Semibold", "PingFangSC_Regular", system-ui', weight: '500' },
  system: { family: 'system-ui, -apple-system, sans-serif', weight: 'bold' },
};

const updateSiteFont = (val) => {
  const config = fontConfigs[val];
  if (config) {
    document.documentElement.style.setProperty('--site-font-family', config.family);
  }
};

const updateLyricFont = (val) => {
  const config = fontConfigs[val];
  if (config) {
    document.documentElement.style.setProperty('--lyrics-font-family', config.family);
    document.documentElement.style.setProperty('--lyrics-font-weight', config.weight);
  }
};
...
watch(
  () => siteFont.value,
  (val) => updateSiteFont(val),
);
watch(
  () => lyricFont.value,
  (val) => updateLyricFont(val),
);
...
provide("updateLyricFont", updateLyricFont);
provide("updateSiteFont", updateSiteFont);

onMounted(() => {
  // 挂载方法
  window.$changeThemeColor = changeThemeColor;
  window.$updateSiteFont = updateSiteFont;
  window.$updateLyricFont = updateLyricFont;
  // 字体设置
  if (siteFont.value) updateSiteFont(siteFont.value);
  if (lyricFont.value) updateLyricFont(lyricFont.value);
});
</script>
