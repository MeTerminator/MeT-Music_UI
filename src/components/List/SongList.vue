...
<template>
  <Transition name="fade" mode="out-in" @after-enter="checkHasPlaying">
    <div v-if="data?.[0]?.id" class="song-list">
      ...
      <n-card v-for="(item, index) in data.slice(
        ...
        <div class="info">
          <div class="title">
            ...
            <div class="tags-wrap">
              <n-tag v-if="item.ttml === 1" :bordered="false" type="info" size="small" round>
                TTML
              </n-tag>
              <n-tag v-if="showPrivilege && item.fee === 1 && userData.detail?.profile?.vipType !== 11"
          ...
          <div class="artist">
            <template v-if="Array.isArray(item.artists)">
              <n-text v-for="ar in item.artists" :key="ar.id" class="ar"
                @click.stop="router.push(`/artist?id=${ar.id}`)" @dblclick.stop>
                {{ ar.name }}
              </n-text>
            </template>
            <template v-else>
      ...
      <!-- 移动端菜单 -->
      <SongListDrawer ref="songListDrawerRef" @playSong="playSong" @delLocalSong="delLocalSong" />
      <!-- 定位歌曲 -->
...
// 当前索引
const finalLoadSize = computed(() => props.loadSize > 0 ? props.loadSize : loadSize.value);
const songsIndex = computed(() => {
  const page = props.showPagination
    ? pageNumber.value
    : Number(router.currentRoute.value.query?.page) || 1;
  return (page - 1) * finalLoadSize.value;
});
