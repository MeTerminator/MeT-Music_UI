...
<template>
  <Transition name="fade" mode="out-in">
    <div v-show="playerControlShow" class="control" @mousemove="controlMove" @mouseenter="controlEnter">
      <div class="left">
        <!-- 下载 -->
      ...
      <div class="center">
        <div class="btn">
          ...
          <!-- 播放暂停 -->
          <n-button :loading="playLoading" :keyboard="false" :focusable="false" class="play-control" strong secondary
            circle @click.stop="playOrPause">
            <template #icon>
              <Transition name="fade" mode="out-in">
        ...
        <div class="slider">
          ...
          <vue-slider v-model="playTimeData.bar" :use-keyboard="false" tooltip="none" width="100%"
            @drag-start="fadePlayOrPause('pause')" @drag-end="sliderDragEnd"
      ...
      <div class="right">
        <!-- MV -->
        <n-icon v-if="music.getPlaySongData.mv" class="hidden" size="22" @click.stop="
          (showFullPlayer = false), router.push(`/videos-player?id=${music.getPlaySongData.mv}`)
          ">
          <SvgIcon icon="video" />
        </n-icon>
        <!-- 评论 -->
        <n-icon v-if="!music.getPlaySongData?.path" class="hidden" size="22" @click.stop="ShowComments()">
        ...
        <n-icon v-if="playMode === 'normal'" class="hidden" size="22" @click.stop="togglePlayMode">
          <SvgIcon :icon="playSongMode === 'normal'
              ? 'repeat-list'
              : playSongMode === 'random'
                ? 'shuffle'
                : 'repeat-song'
