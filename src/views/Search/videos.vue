  ...
  <div class="search-videos">
    <Transition name="fade" mode="out-in">
      <div v-if="searchData !== 'empty'" class="list">
        <!-- 列表 -->
        <MainCover :data="searchData" columns="1 s:2 m:3 l:4 xl:5" type="mv" />
        <!-- 分页 -->
        <Pagination
      ...
      <n-empty
        v-else
        :description="`很抱歉，未能找到与 ${searchKeywords} 相关的任何视频`"
...
const getSearchResData = (
  ...
  try {
    ...
    getSearchRes(keywords, limit, offset, type).then((res) => {
      ...
      // 处理数据
      searchData.value = formatData(res.result.mvs, "mv");
    });
  } catch (error) {
