  ...
  <div class="search-songs">
    <Transition name="fade" mode="out-in">
      <div v-if="searchData !== 'empty'" class="list">
        <!-- 列表 -->
        <SongList :data="searchData" :showPagination="false" :loadSize="settings.searchLoadSize" />
        <!-- 分页 -->
        <Pagination
      ...
      <n-empty
        v-else
        :description="`很抱歉，未能找到与 ${searchKeywords} 相关的任何单曲`"
...
const getSearchResData = (
  ...
  try {
    ...
    getSearchRes(keywords, limit, offset, type).then((res) => {
      ...
      if (res.result.songCount === 0) return (searchData.value = "empty");
      // 处理数据
      searchData.value = formatData(res.result.songs, "song");
    });
  } catch (error) {
  ...
  (val) => {
    if (val.name == "sea-songs") {
      ...
      getSearchResData(
        ...
        settings.searchLoadSize,
        (pageNumber.value - 1) * settings.searchLoadSize,
      );
    }
  },
);
