/**
 * 颜色工具函数(移植自旧 src/utils/color-utils.js,逻辑不变,仅类型化)
 */
 ...
 * @returns 一个包含色调(H)、饱和度(S)和亮度(L)的数组,值的范围均为 [0, 1]
 */
export const rgb2Hsl = ([r, g, b]: readonly number[]): HslColor => {
  // 将 RGB 值转换为范围 [0, 1] 的值
  r /= 255;
  g /= 255;
  b /= 255;
...
/**
 * 将 HSL(色相、饱和度、亮度)颜色值转换为 RGB 颜色值
 * @param hsl - 包含色相(0-1)、饱和度(0-1)和亮度(0-1)的数组
 * @returns 包含红色、绿色和蓝色分量的 RGB 颜色值数组(0-255)
 */
export const hsl2Rgb = ([h, s, l]: readonly number[]): RgbColor => {
 ...
 */
export const normalizeColor = ([r, g, b]: readonly number[]): RgbColor => {
  ...
  // 根据亮度调整颜色
  if (luminance < 60) {
    [r, g, b] = [r, g, b].map((c) => mix(c, 255, 0.3 * (1 - luminance / 60))) as RgbColor;
  } else if (luminance > 180) {
    [r, g, b] = [r, g, b].map((c) => mix(c, 0, 0.5 * ((luminance - 180) / 76))) as RgbColor;
  }
  // 将RGB颜色值转换为HSL颜色值
 ...
 */
export const calcWhiteShadeColor = ([r, g, b]: readonly number[], p = 0.5): RgbColor => {
  // 辅助函数,用于混合两个值
  const mix = (a: number, b: number, p: number): number => Math.round(a * (1 - p) + b * p);
  // 将输入颜色的每个分量与255进行混合,改变亮度
  return [r, g, b].map((c) => mix(c, 255, p)) as RgbColor;
};
 ...
 */
export const calcLuminance = (color: readonly number[]): number => {
  // 将颜色值从0-255映射到0-1范围
  let [r, g, b] = color.map((c) => c / 255) as RgbColor;
  // 对每个分量进行 gamma 校正
  [r, g, b] = [r, g, b].map((c) => {
    if (c <= 0.03928) {
      return c / 12.92;
 ...
 */
export const rgb2Lab = (color: readonly number[] | null | undefined): LabColor => {
  ...
  // 将RGB颜色值映射到0-100范围
  [r, g, b] = [r, g, b].map((c) => c * 100) as RgbColor;
  // 计算XYZ颜色值
...
export const calcColorDifference = (
  color1: readonly number[] | null | undefined,
  color2: readonly number[] | null | undefined,
): number => {
  // 将颜色值转换为LAB颜色值
 ...
 */
export const getGradientFromPalette = (palette: number[][]): string => {
  // 根据亮度对调色板进行排序
 ...
 */
export const argb2Rgb = (x: number): RgbColor => {
  // 提取红色、绿色和蓝色分量
  const r = (x >> 16) & 0xff;
  const g = (x >> 8) & 0xff;
  const b = x & 0xff;
